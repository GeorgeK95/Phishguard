...
export class RecommendationAgent {
  ...
  async handle(request, ontology) {
    ...
    if (detectedFeatures.length === 0) {
      ...
      explanationParts.push(`Detected ${detectedFeatures.length} risk indicator(s): ${detectedFeatures.map((feature) => feature.label).join(', ')}.`);
    }

    if (threatType) {
      explanationParts.push(`Most likely threat type: ${threatType.label}.`);
    }
